...
def run_pipeline(
    ...
    top_k=10,
    corr_threshold=0.85,
    heatmap_path="corr_merged_after.png",
    tree_plot_path="decision_tree.png",
    tree_rules_path="decision_tree_rules.txt"
):
    ...
    ranking = rank_features_by_gain_ratio(
        X_train_s, y_train, feature_names, categorical_unique_threshold=20
    )

    print(f"\nТОП-{len(ranking)} признаков по Gain Ratio:")
    for i, (fname, gr, thr) in enumerate(ranking, start=1):
        if thr is None:
            print(f"{i:2d}) {fname:25s} GainRatio={gr:.6f}  split=categorical")
        else:
            print(f"{i:2d}) {fname:25s} GainRatio={gr:.6f}  best_threshold={thr:.6f}")

    selected_features = [name for name, _, _ in ranking[:top_k]]
    selected_idx = [feature_names.index(f) for f in selected_features]

    X_train_top = X_train_s[:, selected_idx]
    X_test_top = X_test_s[:, selected_idx]
    ...
    with open(tree_rules_path, "w", encoding="utf-8") as f:
        f.write(tree_rules)

    print(f"\nДерево сохранено в файл: {tree_plot_path}")
    print(f"Правила дерева сохранены в файл: {tree_rules_path}")
    ...
    ranking_df = pd.DataFrame(ranking, columns=["feature", "gain_ratio", "best_threshold"])
    return ranking_df, selected_features, clf


if __name__ == "__main__":
    ranking_df, selected_features, clf = run_pipeline(
        red_path="winequality-red.csv",
        white_path="winequality-white.csv",
        top_k=12,
        corr_threshold=0.85,
